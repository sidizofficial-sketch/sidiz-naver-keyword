...
with st.sidebar:
    st.header("⚙️ 기본 설정")
    sid = st.text_input("Sheet ID", "1JnEKEe7HDbN5NG8l0kZ55Rtihp9SBbauD0CzhKQX-qM", key="sheet_id_input")
    unit = st.radio("집계 단위", ["일자별", "주차별", "월별"], index=2, key="time_unit_radio")
    s_date = st.date_input("시작일", datetime(2024, 12, 1), key="start_date_input")
    e_date = st.date_input("종료일", datetime(2025, 1, 31), key="end_date_input")
    
    if s_date >= e_date:
        st.error("시작일은 종료일보다 이전이어야 합니다")
    
    if st.button("🔄 전체 새로고침", key="refresh_button"): 
        st.cache_data.clear()
        st.rerun()
...
# --- 6. 분석 실행 ---
if final_filter:
    if st.button("🚀 분석 시작", type="primary", use_container_width=True, key="start_analysis_button"):
        results = []
